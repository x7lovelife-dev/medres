...
目标是让用户在 Codex 中输入一个医药研究问题后，系统能像一个小型分析团队一样完成：问题澄清、研究计划、数据检索、证据抽取、主题分析、图表建议、Markdown 报告和引用附录。
...
1. 识别药物、适应症、比较范围和结局指标。
2. 检索 ClinicalTrials.gov、PubMed、PMC、openFDA Label 和 Crossref。
3. 抽取试验设计、分期、入组、终点、疗效、安全性和标签信息。
4. 输出临床试验对比表、疗效证据表、安全性差异和综合判断。
...
1. 判断任务是否属于医药深度调研。
2. 分类为靶点竞争格局、药物临床对比或不良事件分析。
3. 按需读取角色卡、报告模板、数据源地图和证据规则。
4. 生成研究计划。
5. 调用 MCP 或可用 API 检索数据。
6. 生成报告并输出引用附录。
...
MCP 服务目录为 `mcp-server/`，可导入代码位于 `mcp_server/`。当前已实现第一个工具 `search_literature` 的 Python 逻辑和 MCP 入口；其他工具仍处于契约规划阶段。
...
- `fetch_publication_detail(ids, source)`
- `search_clinical_trials(condition, intervention, phase, status)`
- `search_adverse_events(drug, reaction, date_range)`
- `search_drug_labels(drug)`
- `normalize_entities(text, entity_types)`
- `rank_evidence(items, research_question)`
...
| V1 | 文献和临床真实查询 | PubMed/Europe PMC/Crossref 已实现；ClinicalTrials 待实现 |
| V2 | 安全性和标签查询 | openFDA FAERS/openFDA Label 工具 |
| V3 | 多源去重和证据排序 | normalize_entities/rank_evidence 工具 |
| V4 | 团队复用 | 打包为 Codex Plugin |
